...
class ResidualAdapter(nn.Module):
    def __init__(self, channels: int, bottleneck_ratio: int = 4, scale_init: float = 0.1):
        super().__init__()
...
class LoRAConv2d(nn.Module):
    def __init__(
        ...
        self.rank = int(rank)
        self.scaling = float(lora_alpha) / max(self.rank, 1)

        self.lora_A = nn.ModuleList(
            [nn.Conv2d(in_channels, self.rank, kernel_size=1, bias=False) for _ in range(num_tasks)]
        )
...
class ConvBlockResidualFocus(nn.Module):
    def __init__(
        self,
        in_ch: int,
        out_ch: int,
        num_tasks: int,
        block_index: int,
        active_residual_blocks: Sequence[int],
        active_lora_blocks: Sequence[int],
        ...
        super().__init__()
        self.use_adapter = peft_mode == "residual" and block_index in set(active_residual_blocks)
        self.use_lora = peft_mode == "lora" and block_index in set(active_lora_blocks)
...
class Task7ResidualFocusCnn14(nn.Module):
    def __init__(
        self,
        sample_rate: int,
        window_size: int,
        ...
        bn_alpha: float = 0.7,
        memory_momentum: float = 0.9,
        adapter_blocks: Optional[Sequence[int]] = None,
        lora_blocks: Optional[Sequence[int]] = None,
        adapter_bottleneck_ratio: int = 4,
        adapter_scale_init: float = 0.1,
    ):
        super().__init__()
        ...
        if lora_blocks is None:
            lora_blocks = [5]
        self.adapter_blocks = list(adapter_blocks)
        self.lora_blocks = list(lora_blocks)
        ...
        self.blocks = nn.ModuleList(
            [
                ConvBlockResidualFocus(
                    channels[i],
                    channels[i + 1],
                    num_tasks=num_tasks,
                    block_index=i,
                    active_residual_blocks=self.adapter_blocks,
                    active_lora_blocks=self.lora_blocks,
                    peft_mode=peft_mode,
                    bn_mode=bn_mode,
                    lora_rank=lora_rank,
    ...
    def route_task(self, waveform: torch.Tensor, seen_task_ids: List[int], strategy: str = "entropy") -> torch.Tensor:
        if strategy != "entropy":
            raise ValueError("This runner supports entropy routing only.")
