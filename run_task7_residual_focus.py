...
from domain_net_task7_residual_focus import Task7ResidualFocusCnn14
...
class RunLogger:
    def __init__(self, args_dict: dict, exp_id: Optional[str] = None, base_dir: str = "logs_residual_focus"):
        self.timestamp = now_str()
        self.exp_id = exp_id if exp_id is not None else f"RES-{self.timestamp}"
        ...
        self.summary = {
            ...
            "avg_oracle_acc": None,
            "avg_routed_acc": None,
            "per_domain_oracle_acc": {},
            "per_domain_routed_acc": {},
    ...
    def set_metrics(self, domain: str, oracle: float, routed: float, route: float) -> None:
        self.summary["per_domain_oracle_acc"][domain] = float(oracle)
        self.summary["per_domain_routed_acc"][domain] = float(routed)
        self.summary["per_domain_route_acc"][domain] = float(route)

    def finish(self, avg_oracle_acc: float, avg_routed_acc: float, status: str = "done") -> None:
        self.summary["avg_oracle_acc"] = float(avg_oracle_acc)
        self.summary["avg_routed_acc"] = float(avg_routed_acc)
        ...
        row = {
            ...
            "avg_oracle_acc": self.summary["avg_oracle_acc"],
            "avg_routed_acc": self.summary["avg_routed_acc"],
        }
        for k, v in self.summary["per_domain_oracle_acc"].items():
            row[f"oracle_{k}"] = v
        for k, v in self.summary["per_domain_routed_acc"].items():
            row[f"routed_{k}"] = v
        for k, v in self.summary["per_domain_route_acc"].items():
            row[f"route_{k}"] = v
...
def compute_anchor_losses(embedding: torch.Tensor, target_idx: torch.Tensor, model: Task7ResidualFocusCnn14):
...
class ResidualFocusLearner:
    def __init__(self, args: argparse.Namespace, logger: RunLogger):
        ...
        self.model = Task7ResidualFocusCnn14(
            ...
            adapter_blocks=parse_blocks(args.adapter_blocks),
            lora_blocks=parse_blocks(args.lora_blocks),
            adapter_bottleneck_ratio=args.adapter_bottleneck_ratio,
            adapter_scale_init=args.adapter_scale_init,
        )
        self.cur_task = -1
        self.prev_model: Optional[Task7ResidualFocusCnn14] = None
    ...
    def _update_reference_geometry(self):
        anchors = F.normalize(self.model.class_anchors.detach(), dim=-1)
    ...
    def incremental_train(self, train_loader, task_name: str, device: str):
        ...
        optimizer = optim.Adam(trainable_params, lr=lr, betas=(0.9, 0.999), eps=1e-8)
        ...
        for epoch_idx in range(1, self.args.epoch + 1):
            ...
            pbar = tqdm(train_loader, desc=f"train {task_name} ep {epoch_idx}/{self.args.epoch}", leave=False)
            for audio, target, _ in pbar:
                ...
                if self.reference_anchor_dist is not None:
                    cur_anchor_dist = pairwise_distance_matrix(F.normalize(self.model.class_anchors, dim=-1))
                ...
                acc = 100.0 * (torch.argmax(logits, dim=1) == target_idx).float().mean().item()

                for k, v in {
                    "loss": total_loss.item(),
                    "ce": ce_loss.item(),
                    "pull": anchor_dict["anchor_pull"].item(),
                    "sep": anchor_dict["anchor_sep"].item(),
                ...
                denom = max(stats["n"], 1)
                pbar.set_postfix(
                    loss=f"{stats['loss']/denom:.3f}",
                    ce=f"{stats['ce']/denom:.3f}",
                    acc=f"{stats['acc']/denom:.2f}",
                )
            ...
            payload = {
                ...
                "batch_acc": stats["acc"] / denom,
            }
            self.logger.log_epoch(payload)
            print(
                f"[{task_name}] ep={epoch_idx:03d} "
                f"loss={payload['total_loss']:.4f} ce={payload['ce_loss']:.4f} "
                f"pull={payload['anchor_pull']:.4f} sep={payload['anchor_sep']:.4f} "
                f"dist={payload['distill_loss']:.4f} struct={payload['structure_loss']:.4f} "
    ...
    @torch.no_grad()
    def evaluate_domain(self, loader, task_id: int, device: str) -> float:
        self.model.to(device)
        self.model.eval()
        correct, total = 0, 0
        for inputs, targets, _ in tqdm(loader, desc=f"oracle task={task_id}", leave=False):
    ...
    @torch.no_grad()
    def evaluate_routed(self, loader, seen_task_ids: List[int], true_task_id: int, device: str):
        self.model.to(device)
        self.model.eval()
        correct, total, route_correct = 0, 0, 0

        for inputs, targets, _ in tqdm(loader, desc=f"routed true={true_task_id}", leave=False):
            inputs = inputs.float().to(device)
            targets = targets.float().to(device)

            routed_task_ids = self.model.route_task(inputs, seen_task_ids=seen_task_ids, strategy="entropy")
        ...
        routed_acc = 100.0 * correct / max(total, 1)
        route_acc = 100.0 * route_correct / max(total, 1)
        return routed_acc, route_acc

    def run(self, task_order: List[str], device: str):
        ...
        for domain_name in task_order:
            ...
            print(f"\n[Train] task_id={self.cur_task} domain={domain_name}", flush=True)
            ...
            print(f"[Eval after {domain_name}]", flush=True)
            for eval_tid, eval_domain in enumerate(task_order[: self.cur_task + 1]):
                ...
                oracle_acc = self.evaluate_domain(test_loader, eval_tid, device=device)
                routed_acc, route_acc = self.evaluate_routed(test_loader, seen_task_ids, true_task_id=eval_tid, device=device)

                self.logger.set_metrics(eval_domain, oracle_acc, routed_acc, route_acc)
                oracle_accs.append(oracle_acc)
                routed_accs.append(routed_acc)

                print(
                    f"  {eval_domain}: oracle={oracle_acc:.2f} routed={routed_acc:.2f} route={route_acc:.2f}",
                    flush=True,
                )

        avg_oracle = float(np.mean(oracle_accs)) if oracle_accs else 0.0
        avg_routed = float(np.mean(routed_accs)) if routed_accs else 0.0
        self.logger.finish(avg_oracle_acc=avg_oracle, avg_routed_acc=avg_routed, status="done")
        print(f"\n[Done] avg_oracle={avg_oracle:.2f} avg_routed={avg_routed:.2f}", flush=True)


def build_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Residual-focus DCASE Task7 runner")
    parser.add_argument("--preset", type=str, default="residual_late2", choices=list(PRESETS.keys()) + ["all"])
    parser.add_argument("--task_order", type=str, default="D2,D3")
    ...
    parser.add_argument("--adapter_blocks", type=str, default="4,5")
    parser.add_argument("--lora_blocks", type=str, default="5")
    parser.add_argument("--adapter_bottleneck_ratio", type=int, default=8)
    parser.add_argument("--adapter_scale_init", type=float, default=0.05)
    ...
    parser.add_argument("--peft_mode", type=str, default="residual", choices=["none", "residual", "lora"])
...
def run_one(args: argparse.Namespace, preset_name: str) -> None:
    ...
    args.adapter_blocks = preset["adapter_blocks"]
    args.lora_blocks = preset["lora_blocks"]
    args.adapter_scale_init = preset["adapter_scale_init"]
    args.adapter_bottleneck_ratio = preset["adapter_bottleneck_ratio"]
    if preset["lora_rank"] > 0:
    ...
    try:
        learner = ResidualFocusLearner(args, logger)
        device = "cuda" if torch.cuda.is_available() else "cpu"
        learner.run(task_order=task_order, device=device)
    except Exception as e:
        logger.summary["error"] = repr(e)
        logger.finish(avg_oracle_acc=0.0, avg_routed_acc=0.0, status="failed")
        raise
