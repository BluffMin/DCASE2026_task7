...
# D1 데이터가 없다는 조건을 반영하여 최종 router/prototype/calibration은 기본적으로 D2/D3만 사용
DEFAULT_ROUTING_TASKS = [1, 2]
...
class Task7AugDataset(Dataset):
    ...
    mixup은 Dataset이 아니라 training loop에서 적용한다.
    """
    def __init__(
        self,
        df: pd.DataFrame,
...
class MCnn14LateBNAdapter(MCnn14):
    ...
    def _forward_block(self, block: nn.Module, x: torch.Tensor, task: int, block_idx: int) -> torch.Tensor:
        ...
        # conv2 + BN + ReLU 이후, pooling 이전에 adapter 삽입
...
def train_one_epoch_adapter(
    ...
    teacher_model: Optional[MCnn14LateBNAdapter] = None,
    ...
    if teacher_model is not None:
        teacher_model.eval()
        teacher_model.set_active_adapters(ADAPTER_BLOCKS)
    ...
    for audio, target, _ in loader:
        ...
        if use_mixup:
            ...
            student_logits = model(mixed_audio, task=task_id)
            ce_loss = lam * criterion(student_logits, y_a) + (1.0 - lam) * criterion(student_logits, y_b)
            ...
            if teacher_model is not None and teacher_task_id is not None and lwf_lambda > 0:
                with torch.no_grad():
                    teacher_logits = teacher_model(mixed_audio, task=teacher_task_id)
                kd_loss = kd_kl_loss(student_logits, teacher_logits, kd_temperature)

        else:
            student_logits = model(audio, task=task_id)
    ...
    avg_ce = round(total_ce / n, 6)
    avg_kd = round(total_kd / n, 6)
...
def train_adapter_for_domain(
    ...
    print("\n" + "=" * 100)
    print(f"[TRAIN] {domain} adapter | aug={aug_mode}, mixup={use_mixup}, lwf_lambda={lwf_lambda}")
    ...
    params = configure_task_adapter_trainable(model, task_id=task_id, adapter_blocks=ADAPTER_BLOCKS)
    print(f"[trainable params] {count_trainable_params(model)}")

    dataset = Task7AugDataset(df=domain_train, data_root=args.data_root, train=True, aug_mode=aug_mode)
    loader = DataLoader(
        dataset,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        fixed_acc = evaluate_fixed_task(model, domain_test, args.data_root, device, task_id=task_id)
        scheduler.step()
...
def forward_audio_all_tasks(
    ...
    chunks = split_into_chunks(audio, CLIP_SAMPLES)
    logits_by_task, probs_by_task, cal_probs_by_task, features_by_task = {}, {}, {}, {}
    entropies, cal_entropies = {}, {}

    for task_id in seen_tasks:
        chunk_logits, chunk_features = [], []
        for chunk in chunks:
            x = torch.from_numpy(chunk.astype(np.float32)).unsqueeze(0).to(device)
        ...
        T = 1.0 if temperatures is None else float(temperatures.get(task_id, 1.0))
        cal_probs = torch.softmax(logits / T, dim=-1)
...
def collect_logits_for_temperature(
    ...
    return torch.cat(logits_list, dim=0), torch.tensor(labels_list, dtype=torch.long)
...
def fit_branch_temperatures(
    ...
    for task_id in seen_tasks:
        ...
        print("-" * 80)
        logits, labels = collect_logits_for_temperature(model, train_df, data_root, task_id, device, max_calib_samples)
...
def build_prototypes(
    ...
    return {"domain_proto": domain_proto, "class_proto": class_proto, "normalize": normalize}


def cosine_distance(a: torch.Tensor, b: torch.Tensor) -> float:
    a = F.normalize(a.detach().cpu(), dim=0)
    b = F.normalize(b.detach().cpu(), dim=0)
        ...
        for task_id in seen_tasks:
            ...
            for _, proto in prototypes["class_proto"][task_id].items():
                ...
                if min_dist is None or dist < min_dist:
                    min_dist = dist
            distances[task_id] = float(min_dist if min_dist is not None else 999.0)
    else:
...
def get_final_probs_and_route(
    ...
    if policy.startswith("fixed_"):
        ...
        scores = {t: ent[t] for t in seen_tasks}
        if 2 in scores:
            scores[2] = scores[2] - d3_bias
        chosen_task = min(seen_tasks, key=lambda t: scores[t])
        final_probs = probs[chosen_task]
        aux["score"] = {TASK_TO_DOMAIN[t]: scores[t] for t in seen_tasks}

    elif policy == "proto_domain_hard":
        if prototypes is None:
            raise ValueError("prototype policy requires prototypes")
        distances = compute_proto_distances(outputs["features"], prototypes, seen_tasks, proto_type="domain")
        ...
        if prototypes is None:
            raise ValueError("prototype policy requires prototypes")
        distances = compute_proto_distances(outputs["features"], prototypes, seen_tasks, proto_type="class")
        chosen_task = min(seen_tasks, key=lambda t: distances[t])
        final_probs = probs[chosen_task]
        aux["score"] = {TASK_TO_DOMAIN[t]: distances[t] for t in seen_tasks}
    ...
    elif policy == "logit_mean":
        chosen_task = -1
        mean_logits = torch.stack([logits[t] for t in seen_tasks], dim=0).mean(dim=0)
        final_probs = torch.softmax(mean_logits, dim=-1)
        aux["score"] = {}

    elif policy in ["entropy_moe", "cal_entropy_moe", "proto_domain_moe", "proto_class_moe"]:
        if policy == "entropy_moe":
            score_dict = {t: ent[t] for t in seen_tasks}
            branch_probs = probs
        elif policy == "cal_entropy_moe":
            score_dict = {t: cal_ent[t] for t in seen_tasks}
            branch_probs = cal_probs
        elif policy == "proto_domain_moe":
            if prototypes is None:
                raise ValueError("prototype policy requires prototypes")
            score_dict = compute_proto_distances(outputs["features"], prototypes, seen_tasks, proto_type="domain")
            branch_probs = probs
        elif policy == "proto_class_moe":
            if prototypes is None:
                raise ValueError("prototype policy requires prototypes")
            score_dict = compute_proto_distances(outputs["features"], prototypes, seen_tasks, proto_type="class")
            branch_probs = probs
        else:
            raise ValueError(policy)

        scores = torch.tensor([score_dict[t] for t in seen_tasks])
        order = torch.argsort(scores)[:min(top_k, len(seen_tasks))]
        selected_tasks = [seen_tasks[i] for i in order.tolist()]
        weights = torch.softmax(-scores[order] / tau, dim=0).to(branch_probs[seen_tasks[0]].device)
        final_probs = 0.0
        for w, t in zip(weights, selected_tasks):
            final_probs = final_probs + w * branch_probs[t]
        chosen_task = selected_tasks[0]
        aux["score"] = {TASK_TO_DOMAIN[t]: score_dict[t] for t in seen_tasks}
...
def evaluate_policy(
    ...
    y_true, y_pred, chosen_tasks, rows = [], [], [], []
    df = df.reset_index(drop=True)
    ...
    for idx, row in df.iterrows():
        ...
        outputs = forward_audio_all_tasks(model, audio, seen_tasks, device, temperatures=temperatures)
        ...
        row_out = {
            ...
            "true_task": true_task,
            "true_domain": domain,
            "route_correct": int(chosen_task == true_task) if chosen_task in TASK_TO_DOMAIN and true_task is not None else np.nan,
            "confidence": conf,
        }
        for task_id in seen_tasks:
        ...
        if (idx + 1) % 100 == 0:
            print(f"[eval] policy={policy}, top_k={top_k}, tau={tau}, bias={d3_bias} | {idx + 1}/{len(df)}")
    ...
    return {"acc": acc, "cm": cm, "route_hist": route_hist, "router_acc": router_acc, "pred_df": pred_df}
...
def save_eval_outputs(result: Dict, out_dir: Path, prefix: str, class_name_map: Dict[int, str]) -> Dict:
    ...
    classwise_accuracy(pred_df, class_name_map).to_csv(out_dir / f"{prefix}_classwise.csv", index=False, encoding="utf-8-sig")
...
def make_policy_configs(args: argparse.Namespace) -> List[Tuple[str, Dict]]:
    configs = []
    for domain in ["D2", "D3"]:
        configs.append((f"fixed_{domain}", {"policy": f"fixed_{domain}"}))
    ...
    configs.append(("logit_mean", {"policy": "logit_mean"}))

    for bias in args.d3_biases:
        name = f"d3_bias_{bias:.2f}".replace(".", "p")
        configs.append((name, {"policy": "d3_bias", "d3_bias": bias}))

    for k in args.top_ks:
        for tau in args.taus:
            tau_name = str(tau).replace(".", "p")
            configs.append((f"entropy_moe_top{k}_tau{tau_name}", {"policy": "entropy_moe", "top_k": k, "tau": tau}))
            configs.append((f"cal_entropy_moe_top{k}_tau{tau_name}", {"policy": "cal_entropy_moe", "top_k": k, "tau": tau}))
            configs.append((f"proto_domain_moe_top{k}_tau{tau_name}", {"policy": "proto_domain_moe", "top_k": k, "tau": tau}))
            configs.append((f"proto_class_moe_top{k}_tau{tau_name}", {"policy": "proto_class_moe", "top_k": k, "tau": tau}))
    return configs
...
def run_policy_suite(
    ...
    args: argparse.Namespace,
    out_dir: Path,
    eval_domains: List[str] = DEFAULT_EVAL_DOMAINS,
    seen_tasks: List[int] = DEFAULT_ROUTING_TASKS,
) -> Dict:
    ensure_dir(out_dir)
    class_name_map = get_class_names(test_df)

    print("\n" + "=" * 100)
    print(f"[1] Fit temperatures | candidates={[TASK_TO_DOMAIN[t] for t in seen_tasks]}")
    print("=" * 100)
    temperatures = fit_branch_temperatures(model, train_df, data_root, seen_tasks, device, args.max_calib_samples)
    save_json({str(k): v for k, v in temperatures.items()}, out_dir / "temperatures.json")

    print("\n" + "=" * 100)
    print(f"[2] Build prototypes | candidates={[TASK_TO_DOMAIN[t] for t in seen_tasks]}")
    print("=" * 100)
    prototypes = build_prototypes(model, train_df, data_root, seen_tasks, device, args.max_proto_samples, normalize=True)
    prototype_info = {
        "domain_proto_norm": {TASK_TO_DOMAIN[k]: float(torch.norm(v).item()) for k, v in prototypes["domain_proto"].items()},
        "class_proto_count": {TASK_TO_DOMAIN[k]: len(v) for k, v in prototypes["class_proto"].items()},
    }
    ...
    for policy_name, cfg in policy_configs:
        ...
        for domain in eval_domains:
            ...
            compact = save_eval_outputs(res, policy_dir, domain, class_name_map)
    ...
    print(summary_df.sort_values("Avg", ascending=False).to_string(index=False))
    print("=" * 100)
    return all_results
...
# Experiment runner
# =========================================================
def parse_aug_setting(mode: str, mixup_alpha: float) -> Dict[str, Any]:
    settings = {
        "none": {"aug_mode": "none", "use_mixup": False, "mixup_alpha": 0.0},
        "crop": {"aug_mode": "crop", "use_mixup": False, "mixup_alpha": 0.0},
        "gain_shift": {"aug_mode": "gain_shift", "use_mixup": False, "mixup_alpha": 0.0},
        "crop_gain_shift": {"aug_mode": "crop_gain_shift", "use_mixup": False, "mixup_alpha": 0.0},
        "crop_gain_shift_noise": {"aug_mode": "crop_gain_shift_noise", "use_mixup": False, "mixup_alpha": 0.0},
        "mixup": {"aug_mode": "none", "use_mixup": True, "mixup_alpha": mixup_alpha},
        "crop_mixup": {"aug_mode": "crop", "use_mixup": True, "mixup_alpha": mixup_alpha},
        "crop_gain_shift_mixup": {"aug_mode": "crop_gain_shift", "use_mixup": True, "mixup_alpha": mixup_alpha},
    }
    if mode not in settings:
        raise ValueError(f"Unknown aug mode: {mode}. Available={list(settings.keys())}")
...
def run_single_study(
    ...
    aug_cfg = parse_aug_setting(aug_name, args.mixup_alpha)
    exp_name = f"symaug_{aug_name}_lwf{str(lwf_lambda).replace('.', 'p')}"
    out_dir = Path(args.save_dir) / exp_name
    ensure_dir(out_dir)

    print("\n" + "#" * 120)
    print(f"[STUDY] {exp_name}")
    print(f"D2/D3 symmetric augmentation: {aug_cfg}")
    ...
    save_json(
        {
            ...
            "routing_tasks": [TASK_TO_DOMAIN[t] for t in DEFAULT_ROUTING_TASKS],
        },
        out_dir / "config.json",
    )
    ...
    # -----------------------------------------------------
    # STEP 1. D2 adapter training
    # LwF 철학: teacher_D1 -> student_D2
    # -----------------------------------------------------
    teacher_d1 = None
    teacher_task_for_d2 = None
    if lwf_lambda > 0:
        teacher_d1 = clone_teacher(model)
        teacher_task_for_d2 = 0
    ...
    d2_result = train_adapter_for_domain(
        ...
        device=device,
        domain="D2",
        out_dir=out_dir / "step1_train_D2",
        aug_mode=aug_cfg["aug_mode"],
        use_mixup=aug_cfg["use_mixup"],
        teacher_model=teacher_d1,
        teacher_task_id=teacher_task_for_d2,
        lwf_lambda=lwf_lambda,
    )
    ...
    # -----------------------------------------------------
    # STEP 2. D3 adapter training
    # LwF 철학: teacher_after_D2의 D2 branch -> student_D3
    # -----------------------------------------------------
    teacher_after_d2 = None
    teacher_task_for_d3 = None
    if lwf_lambda > 0:
        teacher_after_d2 = clone_teacher(model)
        teacher_task_for_d3 = 1
    ...
    d3_result = train_adapter_for_domain(
        ...
        device=device,
        domain="D3",
        out_dir=out_dir / "step2_train_D3",
        aug_mode=aug_cfg["aug_mode"],
        use_mixup=aug_cfg["use_mixup"],
        teacher_model=teacher_after_d2,
        teacher_task_id=teacher_task_for_d3,
        lwf_lambda=lwf_lambda,
    )
    ...
    # -----------------------------------------------------
    # STEP 3. Final D2/D3 routing + calibration + prototype + MoE evaluation
    # -----------------------------------------------------
    final_results = run_policy_suite(
        ...
        eval_domains=DEFAULT_EVAL_DOMAINS,
        seen_tasks=DEFAULT_ROUTING_TASKS,
    )
    save_json(final_results, out_dir / "final_results.json")

    # Compact best row
    rows = []
    for policy_name, r in final_results.items():
        rows.append({
            "exp_name": exp_name,
            "aug_name": aug_name,
            "lwf_lambda": lwf_lambda,
            "policy": policy_name,
            "Avg": r["Avg"],
            "Avg_router_acc": r["Avg_router_acc"],
            "D2_acc": r["D2"]["acc"],
            "D3_acc": r["D3"]["acc"],
            "D2_router_acc": r["D2"]["router_acc"],
            "D3_router_acc": r["D3"]["router_acc"],
        })
    pd.DataFrame(rows).sort_values("Avg", ascending=False).to_csv(out_dir / "compact_policy_summary.csv", index=False, encoding="utf-8-sig")
...
def main(args: argparse.Namespace) -> None:
    ...
    for aug_name in aug_modes:
        for lwf_lambda in args.lwf_lambdas:
            result = run_single_study(args, train_df, test_df, device, aug_name, lwf_lambda)
            all_results_index[result["exp_name"]] = {
                "aug_name": aug_name,
                "lwf_lambda": lwf_lambda,
                "path": str(Path(args.save_dir) / result["exp_name"]),
            }

            for policy_name, r in result["final_results"].items():
                all_study_summaries.append({
                    ...
                    "D2_router_acc": r["D2"]["router_acc"],
                    "D3_router_acc": r["D3"]["router_acc"],
                    "D2_route_D2": r["D2"]["route_hist"].get("D2", 0),
                    "D2_route_D3": r["D2"]["route_hist"].get("D3", 0),
                    "D2_route_MIX": r["D2"]["route_hist"].get("MIX", 0),
                    "D3_route_D2": r["D3"]["route_hist"].get("D2", 0),
                    "D3_route_D3": r["D3"]["route_hist"].get("D3", 0),
                    "D3_route_MIX": r["D3"]["route_hist"].get("MIX", 0),
                })

            save_json(all_results_index, save_dir / "all_results_index.json")
            summary_df = pd.DataFrame(all_study_summaries)
            summary_df.sort_values("Avg", ascending=False).to_csv(save_dir / "summary_all_by_avg.csv", index=False, encoding="utf-8-sig")
            summary_df.sort_values("D3_acc", ascending=False).to_csv(save_dir / "summary_all_by_d3.csv", index=False, encoding="utf-8-sig")
            summary_df.sort_values("Avg_router_acc", ascending=False).to_csv(save_dir / "summary_all_by_router_acc.csv", index=False, encoding="utf-8-sig")

    print("\n" + "=" * 120)
    print("[FINAL SUMMARY TOP-30 BY AVG]")
    final_df = pd.DataFrame(all_study_summaries)
    if len(final_df) > 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[0.0],
        help="0.0 means no LwF. Example: --lwf_lambdas 0.0 0.3 0.5 1.0",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If set, initialize D2/D3 BN state from D1 BN state before each domain training.",
    )

    parser.add_argument("--top_ks", type=int, nargs="+", default=[2])
    parser.add_argument("--taus", type=float, nargs="+", default=[0.5, 1.0, 2.0])
    parser.add_argument("--d3_biases", type=float, nargs="+", default=[0.10, 0.15, 0.20, 0.30, 0.50])
    parser.add_argument("--max_calib_samples", type=int, default=-1)
    parser.add_argument("--max_proto_samples", type=int, default=-1)
