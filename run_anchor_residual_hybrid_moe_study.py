...
# D1 데이터가 없다는 조건:
# - D1 train/calibration/prototype에는 절대 사용하지 않음
# - D1 checkpoint는 frozen anchor/fallback expert로만 사용
ANCHOR_TASK = 0
EXPERT_TASKS = [1, 2]          # routing score/prototype/Mahalanobis는 D2/D3 train only
FORWARD_TASKS = [0, 1, 2]      # cache에는 D1 anchor output도 함께 저장
...
def forward_one_audio_all_branches(
    ...
    wav_path: Path,
    device: str,
    seen_tasks: List[int] = FORWARD_TASKS,
    anchor_model: Optional[MCnn14LateBNAdapter] = None,
) -> Dict[str, Any]:
    ...
    for task_id in seen_tasks:
        # D1은 별도 checkpoint에서 로드한 frozen anchor model을 사용할 수 있음.
        # 없으면 adapted model의 task=0 branch를 사용함.
        run_model = anchor_model if (task_id == ANCHOR_TASK and anchor_model is not None) else model
        chunk_logits = []
        chunk_feats = []
        for chunk in chunks:
            x = torch.from_numpy(chunk.astype(np.float32)).unsqueeze(0).to(device)
            feat = run_model.forward_features(x, task=task_id)
            logits = run_model.fc(feat)
...
def build_forward_cache(
    ...
    for idx, row in df.iterrows():
        ...
        outputs = forward_one_audio_all_branches(model, wav_path, device, seen_tasks, anchor_model=anchor_model)
        rows.append({
...
def fit_temperatures_from_cache(cache: List[Dict[str, Any]], device: str, seen_tasks: List[int] = EXPERT_TASKS) -> Dict[int, float]:
    temperatures = {}
    for task_id in seen_tasks:
...
def add_calibrated_outputs(cache: List[Dict[str, Any]], temperatures: Dict[int, float], seen_tasks: List[int] = EXPERT_TASKS) -> None:
    for item in cache:
...
def build_prototypes_and_descriptors(
    ...
    seen_tasks: List[int] = EXPERT_TASKS,
    eps: float = 1e-5,
) -> Dict[str, Any]:
    domain_proto = {}
...
def add_distance_features(cache: List[Dict[str, Any]], proto_desc: Dict[str, Any], seen_tasks: List[int] = EXPERT_TASKS) -> None:
    for item in cache:
        out = item["outputs"]
...
# Score normalization
# =========================================================
def fit_score_stats(cache: List[Dict[str, Any]], score_keys: List[str], seen_tasks: List[int] = EXPERT_TASKS) -> Dict[str, Dict[int, Dict[str, float]]]:
    stats = {}
...
# =========================================================
# Anchor-residual / fallback helpers
# =========================================================
...
def hybrid_expert_weights(
    ...
    return {t: float(w) for t, w in zip(seen_tasks, weights)}, raw_scores


def anchor_residual_probs(
    item: Dict[str, Any],
    weights_by_task: Dict[int, float],
...
def uncertainty_lambda_from_weights(
    weights_by_task: Dict[int, float],
    threshold: float,
    fallback_lambda: float,
) -> float:
    # max weight가 threshold보다 낮으면 router가 애매하다고 보고 D1 fallback을 섞는다.
    max_w = max(weights_by_task.values()) if weights_by_task else 0.0
    return float(fallback_lambda if max_w < threshold else 0.0)
...
def eval_cache_policy(
    ...
    for item in cache:
        ...
        if policy_kind == "fixed_D2":
        ...
        elif policy_kind == "fixed_D1":
            chosen_task = ANCHOR_TASK
            final_probs = hard_probs(item, ANCHOR_TASK)
        elif policy_kind == "all_mean_with_D1":
            all_tasks = [ANCHOR_TASK] + list(seen_tasks)
            weights_by_task = {t: 1.0 / len(all_tasks) for t in all_tasks}
            final_probs = weighted_probs(item, weights_by_task)
            ...
            all_tasks = [ANCHOR_TASK] + list(seen_tasks)
            logits = np.stack([out["logits"][t] for t in all_tasks], axis=0).mean(axis=0)
            final_probs = probs_from_logits_np(logits)
        elif policy_kind in ["anchor_residual_hybrid", "anchor_residual_fallback"]:
            if score_stats is None:
                raise ValueError(f"{policy_kind} requires score_stats")
            weights_by_task, raw_scores = hybrid_expert_weights(
                item=item,
            ...
            residual_scale = getattr(eval_cache_policy, "_residual_scale", 1.0)
            final_probs = anchor_residual_probs(item, weights_by_task, residual_scale=residual_scale)

            if policy_kind == "anchor_residual_fallback":
                threshold = getattr(eval_cache_policy, "_fallback_threshold", 0.60)
                fallback_lambda = getattr(eval_cache_policy, "_fallback_lambda", 0.50)
                lam = uncertainty_lambda_from_weights(weights_by_task, threshold, fallback_lambda)
                if lam > 0:
                    d1_probs = hard_probs(item, ANCHOR_TASK)
                    final_probs = (1.0 - lam) * final_probs + lam * d1_probs
    ...
    for t in [ANCHOR_TASK] + list(seen_tasks):
...
def build_policy_grid(args: argparse.Namespace) -> List[Tuple[str, Dict[str, Any]]]:
    configs = []
    base = [
        ("fixed_D1_anchor", {"policy_kind": "fixed_D1"}),
    ...
    # D1 is not calibrated or prototyped. It is used only as frozen anchor.
    # D2/D3 routing weights are from the previous best-performing hybrid score.
    for alpha in args.hybrid_alphas:
        for beta in args.hybrid_betas:
            for delta in args.hybrid_deltas:
                for gamma in args.hybrid_gammas:
                    for tau in args.hybrid_taus:
                        for rs in args.anchor_residual_scales:
                            name = (
                                f"anchor_residual_hybrid_a{alpha}_b{beta}_d{delta}_g{gamma}_t{tau}_rs{rs}"
                                .replace(".", "p")
                            )
                            configs.append((name, {
                                "policy_kind": "anchor_residual_hybrid",
                                "alpha": alpha,
                                "beta": beta,
                                "delta": delta,
                                "d3_bias": gamma,
                            ...
                            for th in args.fallback_thresholds:
                                for fl in args.fallback_lambdas:
                                    fname = (
                                        f"anchor_residual_fallback_a{alpha}_b{beta}_d{delta}_g{gamma}_t{tau}_rs{rs}_th{th}_fl{fl}"
                                        .replace(".", "p")
                                    )
                                    configs.append((fname, {
                                        "policy_kind": "anchor_residual_fallback",
                                        "alpha": alpha,
                                        "beta": beta,
                                        "delta": delta,
                                        "d3_bias": gamma,
...
def evaluate_all_policies(
    ...
    for policy_name, cfg in configs:
        ...
        for domain in args.eval_domains:
            ...
            # Optional parameters for anchor-residual/fallback policies.
            eval_cache_policy._residual_scale = cfg.get("residual_scale", 1.0)
            eval_cache_policy._fallback_threshold = cfg.get("fallback_threshold", 0.60)
            eval_cache_policy._fallback_lambda = cfg.get("fallback_lambda", 0.50)
        ...
        avg = round(np.mean([policy_result[d]["acc"] for d in args.eval_domains]), 2)
        avg_router_acc = round(np.nanmean([policy_result[d]["router_acc"] for d in args.eval_domains]), 2)
        row = {"policy": policy_name, "Avg": avg, "Avg_router_acc": avg_router_acc}
        for dname in args.eval_domains:
...
def main(args: argparse.Namespace) -> None:
    ...
    # ------------------------------------------------------------------
    # D1 test set is not available in the current split.
    # Train/calibration/prototype/descriptor construction uses D2/D3 only.
    # Evaluation also follows the available domains only, normally D2/D3.
    # If a missing domain such as D1 is accidentally requested, skip it
    # instead of crashing on an empty confusion matrix.
    # ------------------------------------------------------------------
    requested_eval_domains = list(args.eval_domains)
    available_eval_domains = []
    skipped_eval_domains = []
    for d in requested_eval_domains:
        n = int((test_df_all["domain"] == d).sum())
        if n > 0:
            available_eval_domains.append(d)
        else:
            skipped_eval_domains.append(d)

    if len(available_eval_domains) == 0:
        counts = test_df_all["domain"].value_counts().to_dict()
        raise RuntimeError(
            f"No requested eval domains are available. "
            f"requested={requested_eval_domains}, test_domain_counts={counts}"
        )

    if skipped_eval_domains:
        print(f"[warn] skipped empty eval domains: {skipped_eval_domains}")

    args.eval_domains = available_eval_domains

    # D1 데이터는 없다고 가정하므로 train/calibration/prototype은 항상 D2/D3만 사용한다.
    # D1 checkpoint는 anchor/fallback expert로만 사용한다.
    train_df = train_df_all[train_df_all["domain"].isin(["D2", "D3"])].copy().reset_index(drop=True)
    test_df = test_df_all[test_df_all["domain"].isin(args.eval_domains)].copy().reset_index(drop=True)

    print(f"[data] train total: {len(train_df_all)}")
    print(f"[data] test total : {len(test_df_all)}")
    for d in ["D1", "D2", "D3"]:
        print(f"[data] {d} train={int((train_df_all['domain'] == d).sum())}, test={int((test_df_all['domain'] == d).sum())}")
    print(f"[eval domains] requested={requested_eval_domains}, used={args.eval_domains}")

    class_name_map = get_class_names(test_df_all)

    config = vars(args).copy()
    config["requested_eval_domains"] = requested_eval_domains
    config["skipped_eval_domains"] = skipped_eval_domains
    config["routing_tasks"] = [TASK_TO_DOMAIN[t] for t in EXPERT_TASKS]
    config["eval_domains"] = args.eval_domains
    ...
    anchor_model = build_model(args.adapter_reduction, device)
    load_checkpoint(anchor_model, args.d1_checkpoint, device=device, strict=False)
    anchor_model.eval()
    anchor_model.set_active_adapters(ADAPTER_BLOCKS)
    ...
    print("=" * 100)
    train_cache = build_forward_cache(model, train_df, args.data_root, device, FORWARD_TASKS, cache_name="train_cache", anchor_model=anchor_model)

    print("\n" + "=" * 100)
    print("[2] Fit branch-wise temperatures using D2/D3 train only")
    print("=" * 100)
    temperatures = fit_temperatures_from_cache(train_cache, device=device, seen_tasks=EXPERT_TASKS)
    ...
    score_stats = fit_score_stats(
        ...
        seen_tasks=EXPERT_TASKS,
    )
    save_json(score_stats, out_dir / "score_stats.json")

    print("\n" + "=" * 100)
    print(f"[4] Build test forward cache for {args.eval_domains}")
    print("=" * 100)
    test_cache = build_forward_cache(model, test_df, args.data_root, device, FORWARD_TASKS, cache_name="test_cache", anchor_model=anchor_model)
    ...
    save_json({
        ...
        "D1_test_n": int((test_df["domain"] == "D1").sum()),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", type=str, required=True, help="adapted checkpoint after D2/D3 training, e.g. after_D3.pth or best.pth")
    parser.add_argument("--d1_checkpoint", type=str, required=True, help="frozen D1 checkpoint only; D1 data is not used")
    ...
    parser.add_argument("--eval_domains", type=str, nargs="+", default=DEFAULT_EVAL_DOMAINS, choices=["D1", "D2", "D3"])
    parser.add_argument("--anchor_residual_scales", type=float, nargs="+", default=[0.25, 0.5, 0.75, 1.0])
    parser.add_argument("--fallback_thresholds", type=float, nargs="+", default=[0.55, 0.60, 0.70])
    parser.add_argument("--fallback_lambdas", type=float, nargs="+", default=[0.25, 0.50, 0.75])
